...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.changeStatus, 1000)
  }
  ...
  render() {
    const {isActive, category, score, timer, imgUrl} = this.state
      ...
      <div className="app-container">
        ...
        {!isActive && (
          <div className="first-container">
            <img src={imgUrl} className="large-image" alt="match" />
              ...
              {tabsList.map(eachTab => (
                <li key={eachTab.tabId}>
                  <button
                    type="button"
                    className={`tab-btn ${
                      category === eachTab.tabId ? 'text-highlight' : ''
                    }`}
                    onClick={() => this.clickOnTab(eachTab.tabId)}
                  >
                    {eachTab.displayText}
                  </button>
                </li>
              ...
              {thumbnailList.map(eachObj => (
                <li key={eachObj.id}>
                  <button
                    type="button"
                    className="image-btn"
                    onClick={() => this.clickImage(eachObj.thumbnailUrl)}
                  >
                    <img
